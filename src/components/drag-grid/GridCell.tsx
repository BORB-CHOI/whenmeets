...
export default function GridCell({ date, slot, value, wide, overlayCount, overlayTotal }: GridCellProps) {
  ...
  return (
    <div
      data-date={date}
      data-slot={slot}
      className={`relative w-[44px] h-[20px] border-r border-gray-200 ${CELL_COLORS[value]} transition-colors duration-75 select-none
      ...
      {hasOverlay && (
        <>
          <div
            ...
            style={{ opacity: overlayOpacity }}
          />
          <span className="absolute top-0 right-0.5 text-[8px] text-indigo-500 font-medium leading-[20px] pointer-events-none">
            {overlayCount}
          </span>
        </>
      )}
