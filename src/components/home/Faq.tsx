...
const FAQS = [
  {
    ...
    a: 'when2meet은 2008년에 만들어진 그대로라 모바일에서 드래그가 거의 안 되고, 한국어가 없으며, 이벤트를 만든 후에는 수정도 불가합니다. WhenMeets는 모바일 그리드부터 다시 그렸고, 한국어 + 응답 비밀번호 보호 + 요일 모드 등 한국 모바일 사용자가 부딪히는 마찰 위주로 다듬었습니다.',
  },
  {
    q: 'Timeful 같은 모던 스케줄링 툴과는 어떻게 다른가요?',
    a: 'Timeful은 일정을 입력하려면 로그인이 필요하고, 무료 플랜은 월 3회 제한이 있습니다. WhenMeets는 비로그인으로도 일정 입력 가능, 횟수 제한 없음. 또 결과 그리드가 색 농도뿐 아니라 셀마다 가능한 인원수를 숫자로 함께 표시해서 셀끼리 비교하기 쉽고, 가능한 시간을 모으는 모드 외에 "안 되는 시간"을 모으는 모드도 지원해서 빈 시간이 더 많은 모임에서 입력 부담을 줄일 수 있습니다.',
  },
  {
    q: '이벤트 만들기에 단계가 많지 않나요?',
    a: '단계 없이 모달 하나에서 끝납니다. Doodle, Timeful, Rallly 등 대부분의 일정 조율 도구는 step-by-step 위저드로 4~6단계를 거치는데, 가벼운 약속 잡기엔 그게 오히려 마찰이에요. WhenMeets는 제목·날짜·시간 범위만 한 화면에 입력하면 30초 안에 링크가 만들어집니다.',
  },
  {
    q: '카톡으로 공유한 링크가 "Google 보안 브라우저" 오류를 띄워요.',
    a: 'Google이 카톡·네이버·라인 같은 인앱브라우저에서 OAuth 로그인을 차단하기 때문입니다. WhenMeets는 인앱브라우저를 자동 감지해서 외부 브라우저로 여는 버튼을 띄웁니다. 한 번 크롬이나 사파리에서 열면 그 후로는 정상 작동합니다.',
  },
  {
    q: '"가능"과 "필요하면 가능"을 구분할 수 있나요?',
    a: '네. 응답 시 3단계로 표시할 수 있어요 — 불가 / 필요하면 / 가능. 결과 화면에서도 단계별로 색이 다르게 표시되어 진짜 가능한 시간과 어쩔 수 없을 때 가능한 시간이 구분됩니다. when2meet은 가능/불가 2단계만 있어서 모호한 시간이 어쩔 수 없이 가능 처리됩니다.',
  },
  {
