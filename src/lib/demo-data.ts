...
export const USE_CASE_FIXTURES: Record<DemoSlug, UseCaseFixture> = {
  'wedding-reunion': {
    ...
    scenarioBody: [
      ...
      'DayMeet는 이 **시간 물어보기** 단계를 1분 안에 끝내는 도구입니다. 호스트가 후보 주말 2~3개와 11시부터 21시까지의 시간대를 정해서 링크 하나만 카톡방에 던지면, 친구들은 각자 가능한 시간을 드래그로 표시합니다.',
  ...
  'family-holiday': {
    ...
    scenarioBody: [
      ...
      'DayMeet는 시간 단위 없이 날짜만 받는 **날짜 전용 모드**를 지원합니다. 추석 연휴 4일을 후보로 띄우면, 어머니부터 사촌까지 **가능**과 **불가능**만 탭으로 표시합니다. 손가락 한 번에 한 날짜.',
  ...
  'team-meeting': {
    ...
    scenarioHeadline: '재택과 출근이 섞인 팀에서 **고정 회의 슬롯**을 찾는 가장 빠른 방법',
    scenarioBody: [
      '재택과 출근이 섞인 팀에서는 “언제 다 같이 회의할 수 있느냐”가 매주 골치 아픈 문제입니다. 캘린더 초대를 5번 보내봐야 “그 시간엔 다른 회의가 있다”는 답만 5번 돌아오죠.',
      'DayMeet로 다음 한 주의 평일 5일과 09~18시 시간대를 잡고 링크를 슬랙 채널에 던지면, 팀원은 30초 안에 가능한 슬롯을 드래그합니다. 결과 화면에서 **전원 가능**과 **if needed**까지 한눈에 구분됩니다.',
  ...
  'study-group': {
    ...
    scenarioBody: [
      ...
      'DayMeet로 다음 한 주의 평일 저녁과 주말을 후보 시간대로 잡고 링크를 카톡 단톡방에 공유하면, 스터디원은 회원가입 없이 바로 들어와 가능한 시간을 드래그합니다. 처음 들어온 사람이 비밀번호를 정하면 그 다음부터는 동일한 비밀번호로 자기 응답만 수정 가능합니다.',
