...
- **셀 클릭으로 슬롯 정보 고정** — 데스크탑에서 시간표·달력 셀을 클릭하면 그 슬롯의 응답자 정보와 popover가 화면에 고정된다. 마우스를 다른 곳으로 옮겨도 유지되어 응답자 목록을 차분히 살펴볼 수 있다. 같은 셀 재클릭 / 그리드 바깥 클릭 / 편집 모드 진입 시 해제된다.

### Changed

- **셀 인터랙션 토글화** — 모바일에서 같은 셀을 다시 탭하면 슬롯 시트가 닫히고, 다른 셀을 탭하면 전환된다. 시간표·달력 그리드 동일.
- **히트맵 셀 숫자 색** — 셀 안 인원수 숫자를 진한 옥색으로 바꿔 브랜드 컬러와 통일.
- **편집 모드 점선 테두리** — 다른 사람이 선택한 셀의 점선 테두리를 조금 더 진하게.
- **편집 모드 응답자 목록** — 편집 중에는 응답자 목록 호버 시 체크박스 표기가 뜨지 않는다 (편집 모드에선 필터링이 막혀 있어 무의미했음).

### Fixed

- **데스크탑 셀 클릭 무반응** — 기존엔 셀을 클릭해도 화면에 아무 변화가 없었다. 이제 슬롯 정보가 고정 표시된다.
- **홀드 중 응답자 클릭 시 하이라이트 소실** — 슬롯 고정 상태에서 응답자를 클릭하면 행 하이라이트가 사라지던 문제 수정.
- **홀드 중 스크롤 시 popover 어긋남** — 스크롤하면 popover가 셀에서 떨어지던 문제 수정.
- **그리드 페이지 전환 시 잔여 상태** — 시간표 페이지를 넘기거나 너비가 재측정될 때 슬롯 고정 상태가 어긋나지 않도록 정리.
- **편집 모드 호버 인원수가 unavailable 이벤트에서 틀리던 문제** 수정.
